...
func (cw *ConfigWatcher) reloadConfig(configPath string) {
	log.Info("检测到配置文件变化，重新加载配置", "path", configPath)
...
func (cw *ConfigWatcher) logConfigChanges(oldCfg, newCfg *AppConfig) {
	...
	if oldCfg.Server.Port != newCfg.Server.Port {
		log.Info("服务端口变更", "old", oldCfg.Server.Port, "new", newCfg.Server.Port)
	}

	if oldCfg.Log.Level != newCfg.Log.Level {
		log.Info("日志级别变更", "old", oldCfg.Log.Level, "new", newCfg.Log.Level)
	}

	if oldCfg.Database.MaxOpenConns != newCfg.Database.MaxOpenConns {
		log.Info("数据库最大连接数变更", "old", oldCfg.Database.MaxOpenConns, "new", newCfg.Database.MaxOpenConns)
	}
}
...
func NewHotReloadConfig(configPath string) (*HotReloadConfig, error) {
	...
	viper.WatchConfig()
	viper.OnConfigChange(func(e fsnotify.Event) {
		log.Info("配置文件变化，重新加载", "file", e.Name)

		// 重新解析配置
		newCfg := &AppConfig{}
		if err := viper.Unmarshal(newCfg); err != nil {
			log.Error("解析新配置失败", "error", err)
			return
		}
